...
Plus $\alpha$ proche de 1, plus on accorde de poids au présent.
Plus $\alpha$ proche de 0, plus courbe sera lisse (on ignore les chocs récents).
...
```{r, fig.height=4.5}
mod_add <- hw(AirPassengers, h=12) 
mod_mult <- hw(AirPassengers, h=12, seasonal = "multiplicative") 
autoplot(AirPassengers) + autolayer(mod_add, PI = FALSE, series = "Additif") +
  autolayer(mod_mult, PI = FALSE, series = "Multiplicatif") +
	autolayer(fitted(mod_add), series = "Additif (fitted)") +
	autolayer(fitted(mod_mult), series = "Multiplicatif (fitted)") +
	labs(title  = "Passagers aériens", x = NULL, y = NULL)
```
