...
Un AR modélise l'influence des $p$ réalisations passées sur la réalisation courante : effet mémoire (on a beaucoup vendu la semaine dernière, $X_{t-1}$ on va sûrement beaucoup vendre cette semaine, $X_t$)
...
Résulte d'une accumulation non persistante de $q$ chocs indépendants : effet mémoire des erreurs passées (la semaine dernière il s'est passé un évènement bizarre et imprévu, le système est encore un peu perturbé aujourd'hui par contagion)
