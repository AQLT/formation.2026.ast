...
if (length(packages) > 0) {
	install.packages(packages, repos = c('https://aqlt.r-universe.dev', 'https://cloud.r-project.org'))
}
```
...
-   `nottem` température mensuelle moyenne au château de Nottingham.
...
2. Appliquer un modèle ARIMA sur l'ensemble de la série et sur la série tronquée après rupture. 
...
```{r ipi-est-modeles}
arima_ens <- auto.arima(y)
arima_ens
arima_post_covid <- auto.arima(window(y, start = 2021))
arima_post_covid

```
Comparons maintenant les résultats des deux modèles sur 
```{r ipi-comp-prev}
farima <- function(x, h, model_arima){
  forecast(Arima(
    x, 
    order = model_arima$arma[c(1,6,2)],
    seasonal = list(order = model_arima$arma[c(3,7,4)], period = model_arima$arma[5]), 
  ), h = h)
}

e_ens <- tsCV(y, farima, h = 1, initial = 10*12, model_arima = arima_ens)
# Modèle identifié post-COVID mais estimé sur l'ensemble de la période
e_post_covid_ens <- tsCV(y, farima, h = 1, initial = 10*12, model_arima = arima_post_covid)
# Modèle identifié post-COVID et estimé post-COVID
e_post_covid <- tsCV(window(y,start = 2021), farima, h = 1, initial = 2*12, model_arima = arima_post_covid)
...
```
Le le modèle estimé après la période du COVID (tronquage des données) minimise les révisions sur cette période malgré l'instabilité des estimations du fait du faible nombre d'observations.
```{r}
